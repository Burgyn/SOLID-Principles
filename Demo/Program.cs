    ...
    class Program
    {
        static void Main(string[] args)
        {
            ...
            try
            {
                ...
                {
                    var mailTo = args[1];
                    var derpartment = int.Parse(args[2]);

                    SendReport(mailTo, derpartment);
                }
                else if (args[0] == "-i")
                {
                    ImportPeople();
        ...
        private static void SendReport(string mailTo, int derpartment)
        {
            ...
            reportService.SendReport(derpartment, mailTo);
        }
    }
}